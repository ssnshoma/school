<?php
  ...
  $title = "ویرایش";
  $category = "ثبت نمره";
  if (isset($_POST['edit-btn']) and isset($_GET['editid'])) {
    $id = $_GET['editid'];
    $mark = $_POST['mark'];
  ...
  if (isset($_POST['edit-btn']) and isset($_GET['editidD'])) {
    $id = $_GET['editidD'];
    $mark = $_POST['mark'];
    $qry = "UPDATE `monmark` SET `mark`='$mark' WHERE id=$id";
    $run = $pdo->prepare($qry);
    $run->execute();
    header("Location: ../html/check-final-marks.php");
  }
  function findId()
  {
                      ...
                      <div class="d-flex flex-column align-items-center w-75 m-auto my-3">
                        <form action="" class="w-100" method="post">
                          <div class="row flex-row justify-content-center">
                            <?php
                              if (isset($_GET['editid']) or isset($_GET['editidD'])) {
                                $id = findId();
