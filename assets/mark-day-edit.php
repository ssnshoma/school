<?php
  ...
  if (isset($_POST['Date-edit']) and $_POST['data'] != "") {
    ...
    foreach ($row as $row) {
      ...
      $UpdateSql = "UPDATE `monmark` SET `tarikh`='$newDate' WHERE `tarikh`='$lastDate' and `codemeli`='$codemeli'";
  ...
  if (isset($_POST['details-edit']) and $_POST['details'] != "") {
    ...
    foreach ($row as $row) {
      $codemeli = $row['codemeli'];
      $UpdateSql = "UPDATE `monmark` SET `details`='$details' WHERE `tarikh`='$lastDate' and `codemeli`='$codemeli'";
      $res = $pdo->prepare($UpdateSql);
      $res->execute();
    }
    echo '<script>window.history.go(-2)</script>';
  ...
  if (isset($_POST['delete-yes'])) {
    ...
    foreach ($row as $row) {
      $codemeli = $row['codemeli'];
      $deleteSql = "DELETE FROM `monmark` WHERE `tarikh`='$lastDate' and `codemeli`='$codemeli'";
                  ...
                  <div class="card-body p-5" dir="rtl">
                      <form method="post">
                        <h6 class="text-secondary mb-2">
                          آیا مایل به حذف کلی نمرات این روز هستید؟
                        </h6>
